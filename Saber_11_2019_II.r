...
area = function(x = 0, tail, Z){
  z = seq(-3,3,0.01)
  fdp = dnorm(z)
  plot(z, fdp, type = "l", main = paste('Z = ', toString(Z)), xlab = "z", ylab = "dnorm(z)")
  if(tail == 1){
      polygon(c(z[z <= x], x), c(fdp[z <= x], fdp[z == -3]), col = "green")
...
len1 = length(datos_2019_2_clc$ESTU_GENERO)

# Diagrama de Barras por Departamento
plot(x = datos_2019_2_clc$ESTU_DEPTO_RESIDE, xlab = "Departamento" , las = 3, main = "2019-II")
...
mujeres = data.frame(filter(datos_2019_2_clc, ESTU_GENERO == "F"))
hombres = data.frame(filter(datos_2019_2_clc, ESTU_GENERO == "M"))
n_m = dim(mujeres)[1]
n_h = dim(hombres)[1]
...
est_1 = data.frame(filter(datos_2019_2_clc, FAMI_ESTRATOVIVIENDA == "Estrato 1"))
est_6 = data.frame(filter(datos_2019_2_clc, FAMI_ESTRATOVIVIENDA == "Estrato 6"))
n_E1 = dim(est_1)[1]
n_E6 = dim(est_6)[1]
...
Ant = data.frame(filter(datos_2019_2_clc, ESTU_DEPTO_RESIDE == "ANTIOQUIA"))
Bog = data.frame(filter(datos_2019_2_clc, ESTU_DEPTO_RESIDE == "BOGOTÁ"))
n_A = dim(Ant)[1]
n_B = dim(Bog)[1]
...
Cho = data.frame(filter(datos_2019_2_clc, ESTU_DEPTO_RESIDE == "CHOCO"))
n_C = dim(Cho)[1]
...
Vall = data.frame(filter(datos_2019_2_clc, ESTU_DEPTO_RESIDE == "VALLE"))
n_V = dim(Vall)[1]
...
# No hay evidencia suficiente para decir que a los dos departamentos les va 
# distinto respecto al puntaje global


################################################################################
## 3) MODELOS DE REGRESION 2019-II #############################################

# a) x = MATEMATICAS VS y = CIENCIAS NATURALES

x1 = datos_2019_2_clc$PUNT_MATEMATICAS
y1 = datos_2019_2_clc$PUNT_C_NATURALES
...
# y1_gorrito = beta0 + beta1 * x1 = 11.8661 + 0.7186 * x1
summary(mod1)
...
# b) x = LECTURA CRITICA VS y = INGLES

x2 = datos_2019_2_clc$PUNT_LECTURA_CRITICA
y2 = datos_2019_2_clc$PUNT_INGLES
...
# y2_gorrito = beta0 + beta1 * x2 = 5.0810 + 0.8308 * x2
summary(mod2)
...
# c) x = MATEMATICAS VS y = LECTURA CRITICA

x3 = datos_2019_2_clc$PUNT_MATEMATICAS
y3 = datos_2019_2_clc$PUNT_LECTURA_CRITICA
...
# y3_gorrito = beta0 + beta1 * x3 = 19.4619 + 0.6460 * x3
summary(mod3)
...
# d) x = MATEMATICAS VS y = INGLES

x4 = datos_2019_2_clc$PUNT_MATEMATICAS
y4 = datos_2019_2_clc$PUNT_INGLES
...
# y4_gorrito = beta0 + beta1 * x4 = 12.0388 + 0.7188 * x4
summary(mod4)
...
x5 = datos_2019_2_clc$PUNT_LECTURA_CRITICA
y5 = datos_2019_2_clc$PUNT_SOCIALES_CIUDADANAS
...
# y5_gorrito = beta0 + beta1 * x5 = -1.6935 + 0.9187 * x5
summary(mod5)
